...
import {
  Button, Container, Form, Image, Row, Card, Modal, Col,
...
export default function Profile() {
  ...
  useEffect(() => {
    usersRef.get().then((doc) => {
      if (doc.exists) {
        ...
        storage.ref('pictures').child(doc.data().ProfilePicture).getDownloadURL().then((temp) => {
  ...
  return (
    <div>
            ...
            <Container fluid>
              <Row>
                <Col xs={3} className="justify-content-md-center">
                  {profilePictureFlag && <Image style={{ height: '350px', width: '350px' }} roundedCircle fluid src={profilePicture} />}
                  {!profilePictureFlag && <Image id="avatar" src={Avatar} />}
                  <Button
                    fluid style={{
                       width: '200px', marginLeft: '65px', marginTop: '10px', background: '#E84F11', border: '#E84F11',
                     }} onClick={handleShow}
                  >Edit Profile
                  </Button>
                </Col>
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          ...
          <Container className="justify-content-md-center text-center">
            <div>
              {profilePictureFlag && <Image onClick={() => inputRef.current.click()} style={{ height: '80px', width: '80px' }} fluid roundedCircle src={previewPic} />}
              {!profilePictureFlag && <Image onClick={() => inputRef.current.click()} id="avatar" src={Avatar} />}
            </div>
          ...
          <Form onSubmit={handleSubmit} ref={formRef1}>
            <Form.Group id="email">
              ...
              <Form.Control ref={bioRef} placeholder="Say something about yourself!" name="myForm" />
            </Form.Group>
          </Form>
        </Modal.Body>
        <Modal.Footer>
      ...
      <input style={{ display: 'none' }} ref={inputRef} type="file" name="profilePicture" onChange={onImageChange} />
